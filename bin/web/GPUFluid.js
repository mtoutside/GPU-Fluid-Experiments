...
var GPUFluid = function(width,height,cellSize,solverIterations) {
	...
	var tmp = snow_modules_opengl_web_GL.gl.getExtension("OES_texture_float") == null;
	this.textureQuad = gltoolbox_GeometryTools.getCachedUnitQuad();
	var nearestFactory = function(width1,height1) {
		return gltoolbox_TextureTools.createTexture(width1,height1,{ channelType : 6407, dataType : 5126, filter : 9728});
	};
	var params = { channelType : 6407, dataType : 5126, filter : texture_float_linear_supported ? 9729 : 9728};
	this.velocityRenderTarget = new gltoolbox_render_RenderTarget2Phase(width,height,function(width2,height2) {
		return gltoolbox_TextureTools.createTexture(width2,height2,params);
	});
	...
	var params1 = { channelType : 6407, dataType : 5126, filter : texture_float_linear_supported ? 9729 : 9728};
...
GPUFluid.prototype = {
	...
	,clear: function() {
		...
		snow_modules_opengl_web_GL.gl.bindFramebuffer(36160,_this.readFrameBufferObject);
		snow_modules_opengl_web_GL.gl.clearColor(0,0,0,1);
		snow_modules_opengl_web_GL.gl.clear(16384);
		snow_modules_opengl_web_GL.gl.bindFramebuffer(36160,_this.writeFrameBufferObject);
		snow_modules_opengl_web_GL.gl.clearColor(0,0,0,1);
		snow_modules_opengl_web_GL.gl.clear(16384);
		var _this1 = this.pressureRenderTarget;
		snow_modules_opengl_web_GL.gl.bindFramebuffer(36160,_this1.readFrameBufferObject);
		snow_modules_opengl_web_GL.gl.clearColor(0,0,0,1);
		snow_modules_opengl_web_GL.gl.clear(16384);
		snow_modules_opengl_web_GL.gl.bindFramebuffer(36160,_this1.writeFrameBufferObject);
		snow_modules_opengl_web_GL.gl.clearColor(0,0,0,1);
		snow_modules_opengl_web_GL.gl.clear(16384);
...
GPUParticles.prototype = {
	step: function(dt) {
		var _this = this.stepParticlesShader.dt;
		_this.dirty = true;
		_this.data = dt;
	...
	,setCount: function(newCount) {
		...
		if(this.particleData != null) {
			...
			this.particleData = new gltoolbox_render_RenderTarget2Phase(dataWidth,dataHeight,gltoolbox_TextureTools.createFloatTextureRGBA);
		}
		...
		var this1 = new Float32Array(dataWidth * dataHeight * 2);
		var arrayUVs = this1;
		var index;
		var _g1 = 0;
		var _g = dataWidth;
	...
	,set_flowVelocityField: function(v) {
		var _this = this.stepParticlesShader.flowVelocityField;
		_this.dirty = true;
		return _this.data = v;
	}
...
snow_App.main = function() {
    $global.gpu_fluid_main = new Main();
	new snow_Snow($global.gpu_fluid_main);
};
snow_App.prototype = {
...
Main.prototype = $extend(snow_App.prototype,{
	config: function(config) {
		...
		config.window.title = "Red Snapper Game";
	...
	,lowerQualityRequired: function(magnitude) {
		...
		haxe_Log.trace("Average FPS: " + this.performanceMonitor.fpsSample.average + ", lowering quality to: " + Std.string(newQuality),{ fileName : "Main.hx", lineNumber : 318, className : "Main", methodName : "lowerQualityRequired"});
	...
	,higherQualityRequired: function(magnitude) {
		...
		var newQuality = Type.createEnumIndex(SimulationQuality,qualityIndex);
		haxe_Log.trace("Raising quality to: " + Std.string(newQuality),{ fileName : "Main.hx", lineNumber : 338, className : "Main", methodName : "higherQualityRequired"});
		this.set_simulationQuality(newQuality);
		this.updateSimulationTextures();
	}
	,reset: function() {
...
shaderblox_glsl_GLSLTools.injectConstValue = function(src,name,value) {
	...
	while(reg.match(currStr)) {
		...
		if(nameFound) {
			...
			var initializerRangeInRootStr_start;
			var initializerRangeInRootStr_end;
			initializerRangeInRootStr_start = namePos.pos + namePos.len;
			initializerRangeInRootStr_end = namePos.pos + namePos.len + initializerLength;
			var absoluteOffset = src1.length - currStr.length + declarationPos.pos;
			var initializerRangeAbsolute_start;
			var initializerRangeAbsolute_end;
			initializerRangeAbsolute_start = shaderblox_glsl_GLSLTools.compressedToExploded(exploded,initializerRangeInRootStr_start) + absoluteOffset;
			initializerRangeAbsolute_end = shaderblox_glsl_GLSLTools.compressedToExploded(exploded,initializerRangeInRootStr_end) + absoluteOffset;
...
snow_modules_webaudio__$Audio_AudioDataWebAudio.prototype = $extend(snow_types_AudioData.prototype,{
	destroy: function() {
		...
		snow_types_AudioData.prototype.destroy.call(this);
	}
...
snow_systems_assets_AssetAudio.prototype = $extend(snow_systems_assets_Asset.prototype,{
	...
	,reload_from_bytes: function(_bytes,_format) {
		...
		this.loaded = false;
		return new snow_api_Promise(function(resolve,reject) {
			var _load = _gthis.system.app.audio.module.data_from_bytes(_gthis.id,_bytes,_format);
			_load.then(function(_audio) {
				_gthis.set_audio(_audio);
				resolve(_gthis);
			}).error(reject);
...
snow_systems_audio_Audio.prototype = {
	...
	,onevent: function(_event) {
		...
		if(_event.type == 4) {
			this.suspend();
		} else if(_event.type == 5) {
			this.resume();
		} else if(_event.type == 8) {
...
var snow_systems_audio_AudioSource = function(_app,_data) {
	...
	this.app = _app;
	this.data = _data;
	this.source_id = this.app.make_uniqueid();
	this.instances = [];
};
...
snow_systems_audio_AudioSource.prototype = {
	...
	,bytes_to_seconds: function(_bytes) {
		var _word = this.data.bits_per_sample == 16 ? 2 : 1;
	...
	,destroy: function() {
		if(this.destroyed) {
			haxe_Log.trace("i / audiosource / " + "destroying already destroyed source!",{ fileName : "AudioSource.hx", lineNumber : 84, className : "snow.systems.audio.AudioSource", methodName : "destroy"});
			return;
		}
		this.destroyed = true;
	...
	,string_save: function(_key,_value,_slot) {
		...
		if(_string_map == null) {
			...
			if(storage == null) {
				...
				var _appname = _parts.pop();
				var _org = _parts.join(".");
				var _id = "" + _org + "/" + _appname + "/" + _this.app.io.string_save_prefix + "." + _slot;
				_string = storage.getItem(_id);
			}
			if(_string == null) {
		...
		var _this4 = this.module;
		_contents = window.btoa(_contents);
		var _this5 = this.module;
		...
		if(storage1 == null) {
			...
			var _org1 = _parts1.join(".");
			var _id1 = "" + _org1 + "/" + _appname1 + "/" + _this5.app.io.string_save_prefix + "." + _slot;
			storage1.setItem(_id1,_contents);
			return true;
		}
	}
